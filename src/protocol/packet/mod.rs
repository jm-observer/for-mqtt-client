...
/// and number of bytes that make it. Used for remaining length
/// calculation as well as for calculating property lengths
pub fn length(
    stream: Iter<u8>
...
pub fn read_mqtt_bytes(
    ...
    // Prevent attacks with wrong remaining length. This method is
    // used in `packet.assembly()` with (enough) bytes to frame
    // packet. Ensures that reading variable len string or bytes
    // doesn't cross promised boundary with `read_fixed_header()`
    if len > stream.len() {
...
/// Writes remaining length to stream and returns number of bytes for
/// remaining length
pub fn write_remaining_length(
    stream: &mut BytesMut,
    len: usize
) -> usize {
    // only publish packet should be limited and limited at new
    // function now. todo
    // if len > 268_435_455 {
...
/// , It's possible that content itself in the stream is wrong. Like
/// expected packet id or qos not being present. In cases where
/// `read_mqtt_string` or `read_mqtt_bytes` exhausted remaining length
/// but packet framing expects to parse qos next, these pre checks
/// will prevent `bytes` crashes
