...
use crate::tasks::task_client::{Client, ClientRx};
...
impl TaskHub {
    pub async fn connect(
        ...
        let identity_network = bus.simple_login().await?;
    ...
    pub async fn recv(
        ...
        // if let Ok(command) = event.clone().downcast::<ClientCommand>() {
        //     self.deal_client_command_when_connected(command.as_ref())
        //         .await?
        // } else
        if let Ok(command) = event.clone().downcast::<ClientData>() {
            self.deal_client_data_when_connected(command.as_ref().clone(), b)
                .await?
        // } else if let Ok(network_status) = event.clone().downcast::<NetworkEvent>() {
        //     self.update_connected_state_by_network_status(network_status.as_ref())?
        } else if let Ok(hub_msg) = event.downcast::<HubMsg>() {
            self.deal_hub_msg(hub_msg.as_ref(), a, b).await?
        } else {
            return Err(HubError::Other("should not be reached".to_string()));
        }
        Ok(())
    }
    ...
    fn update_connected_state_by_network_status(
        ...
        match state {
            NetworkEvent::ConnectedErr(msg) => {
                ...
                self.identity
                    .dispatch_event(MqttEvent::ConnectedErr(msg.clone()))?;

                self.state = HubState::ToDisconnect(ToDisconnectReason::NetworkErr(msg.clone()));
                // if self.options.auto_reconnect {
                //     self.state = HubState::ToConnect;
                // } else {
                //     self.state = HubState::Disconnected;
    ...
    ///
    async fn run_to_connect(&mut self) -> Result<(), HubToConnectError> {
        let mut first = true;
        loop {
            if !first {
            ...
            match status.as_ref() {
                ...
                NetworkEvent::ConnectFail(reason) => {
                    ...
                    self.identity
                        .dispatch_event(MqttEvent::ConnectFail(reason.clone()))?;
    ...
    async fn deal_hub_msg(
        ...
        match req {
            ...
            HubMsg::PingFail => {
                ...
                self.identity
                    .dispatch_event(MqttEvent::ConnectedErr("ping fail".to_string()))?;
            }
            ...
            HubMsg::RxPublish(publish) => match publish.qos {
                QoSWithPacketId::AtMostOnce => {
                    ...
                    self.identity
                        .dispatch_event(MqttEvent::Publish(publish.clone()))?;
                }
                QoSWithPacketId::AtLeastOnce(id) => {
                    if self.rx_publish_id.contains_key(&id) {
                        ...
                        TaskPublishRxQos1::init(self.bus.clone(), id, self.protocol).await?;
                    }
                }
                QoSWithPacketId::ExactlyOnce(id) => {
                    if self.rx_publish_id.contains_key(&id) {
                        ...
                        TaskPublishRxQos2::init(self.bus.clone(), id, publish.protocol).await?;
                        self.rx_publish.insert(id, publish.clone());
                        self.rx_publish_id.insert(id, id);
                    }
                }
            ...
            HubMsg::AffirmRxPublish(id) => {
                if let Some(publish) = self.rx_publish.remove(&id) {
                    self.identity.dispatch_event(MqttEvent::Publish(publish))?;
                    // self.tx_to_user.send(publish.into())?;
                } else {
    ...
    async fn deal_client_data_when_connected(
        ...
        match req {
            ClientData::Subscribe(mut trace_subscribe) => {
                ...
                TaskSubscribe::init(self.bus.clone(), trace_subscribe.clone()).await?;
            }
            ...
            ClientData::PublishQoS1(mut packet) => {
                ...
                TaskPublishQos1::init(self.bus.clone(), packet).await?;
            }
            ClientData::PublishQoS2(mut packet) => {
                packet.set_packet_id(b).await?;
                self.client_data.push_back(packet.clone().into());
                TaskPublishQos2::init(self.bus.clone(), packet).await?;
            }
        }
        Ok(())
    }
...
fn init_keep_alive_check(time: KeepAliveTime, keep_alive: u16, tx: IdentityOfTx) {
    spawn(async move {
        sleep(Duration::from_secs(keep_alive as u64)).await;
        debug!("keep_alive_check wake {:?}", time);
        if tx.dispatch_event(HubMsg::KeepAlive(time)).is_err() {
