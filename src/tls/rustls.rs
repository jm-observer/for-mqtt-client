...
use rustls::{
    Certificate, ClientConfig, Error, PrivateKey, ServerName,
};
...
pub fn init_rustls(tls_config: TlsConfig) -> Result<TlsConnector> {
    ...
    let builder = match verify_server {
        VerifyServer::CA { verify_dns_name } => {
            let verifier = Arc::new(PkiVerifier::new(
                roots,
                verify_dns_name,
                true,
            ));
        ...
        VerifyServer::SelfSigned {
            ...
            let roots: Vec<rustls_native_certs::Certificate> =
                ca_file
                    .load()?
                    .into_iter()
        ...
        VerifyServer::Insecurity => {
            let verifier =
                Arc::new(PkiVerifier::new(roots, false, false));
            ClientConfig::builder()
                .with_safe_defaults()
                .with_custom_certificate_verifier(verifier)
        },
    };
    let config = match verify_client {
...
impl ServerCertVerifier for PkiVerifier {
    fn verify_server_cert(
        ...
        if self.verify_server {
            let cert = webpki::EndEntityCert::try_from(
                end_entity.0.as_ref(),
            )
            .map_err(pki_error)?;
            ...
            let trustroots: Vec<webpki::TrustAnchor> = self
                ...
                .iter()
                .filter_map(|x| {
                    match webpki::TrustAnchor::try_from_cert_der(&x.0)
                    {
            ...
            if self.verify_dns_name {
                ...
                let dns_name_ref =
                    DnsNameRef::try_from_ascii_str(dns_name.as_ref())
...
fn pki_error(error: webpki::Error) -> Error {
    ...
    match error {
        ...
        e => Error::InvalidCertificateData(format!(
...
type SignatureAlgorithms =
    &'static [&'static webpki::SignatureAlgorithm];
